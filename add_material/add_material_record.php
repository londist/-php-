...
$sql = "insert into add_material (mid,sid,charge,price,amount,last_modified) values ($mate,$supp,$work,$price,$account,now())";

$result = mysql_query($sql,$db);
if (!$result)
    die("添加进货记录失败 !<br/>" . mysql_error());

echo "<script>alert('成功添加一条进货记录');location='./m-add_material.php'</script>";
mysql_close($db);
